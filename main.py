...
def drawRectangles(frame, hor_amount, ver_amout):
    height, width, _ = frame.shape

    for j in range(ver_amout):
        for i in range(hor_amount):
            x1 = int(i * width / hor_amount)
            y1 = int(j * height / ver_amout)

            x2 = int((i + 1) * width / hor_amount)
            y2 = int((j + 1) * height / ver_amout)
...
def eye_long_detect():
    ...
    for face in faces:
        ...
        right_eye_cord = get_eye_cord(landmarks)
